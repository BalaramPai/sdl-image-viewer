...
int main()
{
	...
	Uint32 color = 0;  // Declaring outside the loop.
	
	// Now we fill each pixel.
	for(int y=0 ; y<height ; y++)
	{
		for(int x=0; x<width ; x++)
		{
			...
			r = (char) getchar(); 	// Reads from the file the ASCII value of the Pixel each.
			g = (char) getchar();
			b = (char) getchar();
						
			//Convert RBG values into the colour format used by SDL surface.
			color = SDL_MapRGB(psurface->format,r,g,b);       // psurface->format  is short cut for (*psurface).format
							                  // Here we have used the format of the surface but there are many predefine types like bitsperpixel,bytesperpixel etc.
			
                	// Update the position of each pixel as sixe is constant.
			pixel.x = x;
			pixel.y = y;
